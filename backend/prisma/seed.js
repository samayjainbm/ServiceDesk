// prisma/seed.js  (UPDATED SEED for NEW schema + StockPurchased + DemandStock)
// Run: node prisma/seed.js  (or via prisma "seed" script)

require("dotenv").config();
const prisma = require("../config/db");
const bcrypt = require("bcryptjs");

// helper: convert {a:2,b:0,...} => [{item_id, count}, ...] using itemIdMap
...
async function main() {
  console.log("Seeding started (UPDATED schema + StockPurchased + DemandStock)...");

  // 1) Clear old data (dependency-safe order)
  // NEW tables first (no one depends on them except items FK)
  ...
  // users AFTER complaints (complaints reference users)
  await prisma.user_info.deleteMany();

  // items last (FK referenced by other tables)
  await prisma.items.deleteMany();

  // 2) Seed items (a..p) in NEW format: item_id, item_name, count
  const itemsSeed = [
  ...
  await prisma.items.createMany({ data: itemsSeed, skipDuplicates: true });

  // Build item_name -> item_id map (needed for FK inserts)
  ...
  console.log("✅ items seeded:", itemsInDb.length);
  ...
  await prisma.worker_credentials.createMany({ data: hashedCredentials, skipDuplicates: true });
  console.log("✅ worker_credentials seeded.");

  // 4) user_info (fixed IDs 1..10 + YOU 11)
  // Password rule: User@<phone_number>
  const usersSeedRaw = [
  ...
  await prisma.user_info.createMany({ data: usersSeed, skipDuplicates: true });
  console.log("✅ user_info seeded (includes Priyansh + Google fields).");
  ...
  await prisma.ongoing_complaints.createMany({ data: complaintsHeader, skipDuplicates: true });
  console.log("✅ ongoing_complaints seeded (headers).");

  // 5.1) complaint_items — rows per item
  ...
  for (const [complaint_id, countsObj] of complaintItemCounts.entries()) {
    ...
    for (const r of rows) complaintItemsRows.push({ complaint_id, item_id: r.item_id, count: r.count });
  }

  if (complaintItemsRows.length) {
    await prisma.complaint_items.createMany({ data: complaintItemsRows, skipDuplicates: true });
  }
  console.log("✅ complaint_items seeded:", complaintItemsRows.length);

  // 6) demanded_items — rows per item demanded
  const demandedOld = [
  ...
  for (const d of demandedOld) {
    ...
    for (const r of rows) {
      demandedRows.push({ worker_id: d.worker_id, complaint_id: d.complaint_id, item_id: r.item_id, count: r.count });
    }
  }

  if (demandedRows.length) {
    await prisma.demanded_items.createMany({ data: demandedRows, skipDuplicates: true });
  }
  console.log("✅ demanded_items seeded:", demandedRows.length);

  // 7) worker_debt — rows per item debt
  const workerDebtOld = [
  ...
  for (const w of workerDebtOld) {
    ...
    for (const r of rows) workerDebtRows.push({ worker_id: w.worker_id, item_id: r.item_id, count: r.count });
  }

  if (workerDebtRows.length) {
    await prisma.worker_debt.createMany({ data: workerDebtRows, skipDuplicates: true });
  }
  console.log("✅ worker_debt seeded:", workerDebtRows.length);

  // 8) alloted_task (count assigned complaints per worker)
  ...
  const countMap = new Map(assignedComplaints.map((x) => [x.worker_id, x._count.complaint_id]));
  const allotData = allWorkers.map((w) => ({ worker_id: w.worker_id, alloted_task: countMap.get(w.worker_id) ?? 0 }));
  await prisma.alloted_task.createMany({ data: allotData, skipDuplicates: true });
  console.log("✅ alloted_task seeded.");

  // 9) ✅ NEW: StockPurchased + DemandStock
  // (sample seed - adjust as you want)
  ...
  if (stockPurchasedSeed.some((x) => !x.item_id)) throw new Error("StockPurchased seed: invalid item_name mapping");
  await prisma.StockPurchased.createMany({ data: stockPurchasedSeed, skipDuplicates: true });
  console.log("✅ StockPurchased seeded:", stockPurchasedSeed.length);
  ...
  if (demandStockSeed.some((x) => !x.item_id)) throw new Error("DemandStock seed: invalid item_name mapping");
  await prisma.DemandStock.createMany({ data: demandStockSeed, skipDuplicates: true });
  console.log("✅ DemandStock seeded:", demandStockSeed.length);

  console.log("✅ Seeding completed (UPDATED schema).");
  console.log("   User passwords: User@<phone_number>");
  console.log("   Worker passwords: Worker@<worker_id>");
}
